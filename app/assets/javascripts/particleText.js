(function (a) {
	a.fn.particleText = function (p) {
		...
		function g(r, q) {
			...
			this.r = f.clientWidth / 2 * 0.0025;//数字が大きくなると粒子のサイズが小さくなる
		...
		function h() {
			...
			var w = 11;//文字サイズ変更数字が小さくなると文字が大きくなる
			o.font = "bold " + (
				x / w
			) + "px 'Times New Roman'";//フォントの指定
			o.textAlign = "center";
			if (!e) {
				v()
			} else {
			...
			n = [];
			for (var t = 0; t < x; t += Math.round(x / 400)) {//粒子の数
				for (var s = 0; s < r; s += Math.round(x / 400)) {//粒子の数
					if (u[(
						(t + s * x) * 4
					) + 3] > 150) {
						n.push(new g(t, s))
